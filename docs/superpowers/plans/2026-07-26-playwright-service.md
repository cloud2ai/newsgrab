...
Post-task-review follow-up: the pip mirror invocation originally paired `--trusted-host pypi.tuna.tsinghua.edu.cn` with its `https://` index URL, unconditionally disabling certificate verification for that host. Verified in an isolated container that the mirror's certificate is valid and pip works identically without `--trusted-host`; removed the flag. Also rebuilt the full playwright-service image after this change and re-ran the live smoke test -- the container started, `GET /healthz` returned `{"status":"ok","browser_connected":true}`, and `POST /v1/actions` with `resolve_and_render` against `https://example.com` returned real rendered HTML -- before committing `c476b85`.
