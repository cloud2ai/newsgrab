...
**Infrastructure: fully verified working.**

- `docker compose up -d --build` brought up both containers successfully.
- Both healthchecks passed from inside the compose network: `playwright-service` returned `{"status":"ok","browser_connected":true}`, `collector-service` returned `{"status":"ok"}`.
- **Found and fixed a real bug during this step**: `app/collectors/google_news.py`'s self-registration (`COLLECTORS["google_news"] = collect`) never ran in the actual app, because nothing in `app/main.py`'s import graph reached that module -- only `app/collectors/base.py` was imported, and the `google_news` backend request returned `400 unknown backend` on the first real attempt. Fixed by importing `google_news` inside `app/collectors/__init__.py` (commit `db2db7b`), which runs transitively whenever anything imports from `app.collectors`. Verified post-fix: `COLLECTORS` correctly showed `["echo", "google_news"]` both locally and inside the rebuilt container; full test suite (46/46) unaffected.
- After the fix, `POST /jobs {"backend": "google_news", "query": "贵州茅台 600519", ...}` returned `201` with a real `job_id`; polling `GET /jobs/{id}` correctly transitioned `pending` → `running` → `done` (5 polls, ~15s).

**External network hop: could not be verified in this sandbox.**

- The job completed with `"status": "done"` but `"result": []` -- zero articles, not a job failure.
- Root cause (found via `docker compose logs`): `gnews`'s underlying `feedparser.parse()` call to `news.google.com` never received a real HTTP response (no `.status` attribute at all on the result, which `gnews`'s own code doesn't handle and surfaces as `gnews.exceptions.NetworkError`).
- Diagnosed as a **sandbox-level Docker networking restriction, not a code defect**: confirmed the host shell itself reaches `https://news.google.com` successfully (`200 OK`), but a plain `python:3.11-slim` container on the default bridge network times out on the same request, and the same request **also times out under `--network host`** -- ruling out both the custom `newsgrab-internal` network and Docker's bridge/NAT layer specifically as the cause. This points to a deliberate, broader restriction on Docker-spawned processes' internet egress in this development sandbox (common in agentic coding sandboxes, to bound what spawned containers can reach), which no compose/network configuration change from inside this repo can work around.
- Practical implication: this is specific to *this sandbox*. A normal deployment target (a real server, cloud VM, or a CI runner without this kind of container-egress firewalling) would not have this restriction, and `playwright-service`'s Chromium needs the exact same real internet access to resolve/render actual news sites regardless -- so this isn't a gap unique to `collector-service`'s HTTP client, it would block real content collection at the browser-automation layer too if it were reachable at all.
- **Not independently re-verified in this session**: whether a real `google_news` job, run somewhere with actual internet egress, returns non-empty, on-topic articles for a real stock query. Every component up to that final external hop (job lifecycle, collector registration wiring, gnews's link-collection code path executing without raising, dedup cache, SSRF guard, content parser fallback selection) is covered by passing unit/integration tests and the infrastructure verification above, but the fully-live "does this actually return real 贵州茅台 news" claim remains unverified pending a network-unrestricted environment.

## Correction (2026-07-27): the "sandbox-level Docker networking restriction" conclusion above was wrong

The diagnosis two sections up -- that Docker-spawned containers in this sandbox categorically cannot reach the internet, under any network mode -- was based on incomplete tests: every earlier probe (`docker run` with plain `curl`/`wget`, `--network host`) never configured an HTTP(S) proxy inside the container. This sandbox's outbound internet access requires going through a proxy reachable at the Docker bridge gateway address (visible to the host as `HTTPS_PROXY`/`HTTP_PROXY` env vars); a bare request with no proxy configured fails identically regardless of network mode, which is exactly what those earlier tests showed and is why they were misread as "no egress at all."

Re-verified properly this time: a container on a custom bridge network (matching `newsgrab-internal`'s setup, not just the default bridge) reaches `https://news.google.com` successfully (`200 OK`) once the proxy is explicitly configured. Two real gaps in `playwright-service`/`collector-service` surfaced and were fixed as part of this correction (commit `0b674e1` in this repo):

1. `playwright-service/entrypoint.sh` never read `HTTPS_PROXY`/`HTTP_PROXY` and translated it into Chromium's `--proxy-server` flag -- Chrome does not honor these env vars automatically the way `curl`/`requests` do (this is the same gap newshub's `browserless_client.py` already handles for its own Browserless setup; it was simply missed when `playwright-service` was originally built).
2. `docker-compose.yml` didn't pass `HTTPS_PROXY`/`HTTP_PROXY`/`NO_PROXY` through to either service at all.

A `NO_PROXY`/`no_proxy` value that excludes `localhost`/`127.0.0.1`/`playwright-service`/`collector-service` is required alongside the proxy vars -- without it, internal service-to-service traffic (health checks, `collector-service` → `playwright-service`) gets routed through the external proxy too, which is wrong and in this sandbox actually hit an unrelated backend. `docker-compose.yml`'s `${NO_PROXY:-...}` default now includes the service hostnames explicitly so this holds even in a deployment whose own `NO_PROXY` doesn't happen to cover it.

After both fixes, rebuilt `playwright-service`'s image and ran a real `docker compose up -d` (not a host-process workaround) with the proxy configured: `POST /jobs {"backend": "google_news", "query": "宝钢股份 600019", ...}` against the actual containerized `collector-service` returned `done` with real, on-topic articles (证券之星/搜狐 coverage of 宝钢股份/600019, correct titles/content/URLs/dates) -- the first genuinely real, fully-containerized end-to-end verification of this pipeline. `GOOGLE_NEWS_LANGUAGE`/`GOOGLE_NEWS_REGION` (unset defaults to `en`/`US`) also need to be set to `zh-CN`/`CN` for Chinese-language queries to return anything -- not a bug, just a deployment configuration choice that's easy to forget, now pass-through via `docker-compose.yml` as well.

**Practical implication, corrected**: this sandbox's restriction is proxy-configuration, not a hard egress block, and this is very likely representative of many real deployment environments (corporate networks, CI runners) that route through an egress proxy rather than allowing direct internet access -- so this fix has value beyond just this sandbox.
